...
### 34. GRIMOIRE に「参考文献・外部リソース」セクション
- 別セッション評価指摘: 学術的信頼性を上げるとAI引用率が上がる
- 記事末尾に「参考にした一次資料・書籍・論文」ブロックを設ける
- GRIMOIRE投稿スキル（`/グリモワール投稿`）に引用元提示を組み込むと運用しやすい
- E-E-A-T（#26）とも連動
...
### 32. フッター拡張
- 別セッション評価指摘: 現状フッターはリンク列のみで世界観が出ていない
- 案:
  - キャラ立ち絵（小）＋ 一言紹介
  - サイトコンセプトの2〜3行リード
  - Xボタンを RSS より目立つ位置に（#37 と連動）
- レイアウト変更は共通Layoutに影響するので、優先度は低めだが効果は見える

### 33. GRIMOIRE 長文の可読性チェック
- 別セッション評価指摘: ダーク背景×長文で読み疲れの可能性
- チェック観点:
  - 本文のコントラスト比（WCAG AA: 4.5:1）
  - 行間・段落間のゆとり
  - スマホ実機での横幅・字詰まり
  - ダーク/ライト切替（OS追従、#23 とも連動）の必要性再評価
- 優先度は低だが、読了率に直結するので GRIMOIRE 蓄積（#10）と並行で回すと良い
...
### 37. サイトからXアカウントへの誘導強化
- 別セッション評価指摘: フッターのRSSに比べてX導線が弱い
- 案:
  - ヘッダーに Xアイコンボタン（小さく・常時表示）
  - 各記事末尾に「Xでシェア」＋ `@occult_wire` フォローCTA
  - 「更新はXで告知」をフッターに明記
- 工数小・効果中。早めにやる価値あり
