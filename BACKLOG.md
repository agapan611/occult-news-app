...
### 26. E-E-A-T対応（残: 経歴・実績の継続蓄積）
- 2026-04-21 **運営者情報・訂正ポリシー 対応済**（下部参照）
...
- [x] **`/legal` 運営者情報**
  - 運営元: 「AiNiwa（個人運営）」→「**AiNiwa（運営者: 庭師）**」
  - 関連サイト欄を新設: [ainiwa.jp](https://ainiwa.jp/)（AiNiwa ポータル）
- [x] **`/legal` 訂正ポリシー新セクション追加**（id="correction"、免責事項の直前）
  - 誤り通知先: お問い合わせフォーム or shunaraika@gmail.com
  - 訂正履歴を記事末尾に明記する旨
  - AIキャラ考察は創作、参考文献の誤りは訂正対象と明文化
- [x] **`/about` 末尾に「SITE OPERATOR」セクション追加**
  - 「運営: 庭師（AiNiwa）」+ ainiwa.jp リンク
  - /legal への導線
- 評価スコア見込み: 10. 信頼性・E-E-A-T 72→82（運営者 -5 + 訂正 -3 + 関連サイト -2 回収）/ 14. 法令対応 77→80
...
## 次セッション引き継ぎメモ（2026-04-21 v2 セッション終了時点）

### 今日の実績（2026-04-21 通算）
- **通算 21+ コミット**（`4a98631` 〜 GA4 イベント計測バンドル）
- サイト評価: 2026-04-20 初回 **78.8** → 2026-04-21 v1 **82.0** → 2026-04-21 v2 **84.0**（+2.0、PSI 実測初導入）
- 評価ファイル: `~/.claude/skills/site-evaluation/results/occult-wire/2026-04-21_v2.md`
- **PSI 実測（Playwright MCP 経由）**: モバイル Perf **57** / デスクトップ Perf **89** / A11y 91 / BP 96 / SEO 100
- **#47 フォント preload 削減効果を実測確認**: `/grimoire/shuna-gobekli-tepe-ancient-temple` モバイル LCP **17.3s → 9.3s**（-8s、約 46% 改善）
- **site-evaluation スキル改修**: Playwright MCP で PageSpeed Insights を自動実測するステップを追加済（Step 2b）

### 今日の対応済み（BACKLOG から消化）
#30 Cookie同意 / #41 セキュリティ4ヘッダ / #42a-c パンくず・スクロールトップ・HTMLサイトマップ / #43 404/500 キャラトーン / #44 Vercel Analytics / #45 サイト内検索 / #47 タイポ導入→部分巻戻し / #48 JSON Feed（PUSH除く）/ #49 CSP Report-Only / #50 GA4 実運用化 / #50 イベント計測（本セッション追加）/ #51 関連記事スコア順 / #52 検索ハイライト / #53 利用規約独立 / #10 運営者プロフィール / 収益動線第一弾（離脱防止CTA + アフィリUI土台）

### 次セッション開始時に優先度高いもの

**🔧 私で即着手（海斗さん負担なし）**
- **#4 パフォーマンス根本改善**（モバイル Perf 57→75+ 目標）: `next/dynamic` で関連記事・アフィリセクションを遅延、未使用 CSS/JS 削減、画像最適化見直し（工数中、加点 +3〜5、最優先）
- #23 デザイン世界観強化: カテゴリ色分け / 🔥新着マーク / キャラヒーロー画像（工数小、視覚メリハリ、加点 +1）
- #27 「今日のオカルト度」ゲージ: 日替わり装飾要素（工数小、世界観、加点 +0.5）
- #24 検索オートサジェスト: 既存の `/search` に候補表示追加（工数小、UX 向上、加点 +0.5）
...
1. **#4 パフォーマンス根本改善に着手**（モバイル Perf 57 がサイト全体の最大ボトルネック）
2. パフォ改善後、PSI 再計測（Playwright 自動）で効果を数値確認
3. 残タスクを効率順で着手
