...
- [x] **`--color-muted` を zinc-500 (#71717a) → zinc-400 (#a1a1aa) に変更**
  - コントラスト比: 4.22:1（AA 未達） → **7.74:1**（AAA クリア）
  - 対象箇所: 全24ファイル・67箇所の `text-muted` が一括改善（CSS 変数のおかげ）
  - 代表例: Hero の日付「2026.04.22 (WED)」、スコア分母「/ 100」、Footer コピーライト、各種メタ情報
- [x] **opacity 付き利用箇所の妥当性確認**
  - `text-muted/80` / `text-muted/90`: AA/AAA クリア（zinc-400 を薄めても 6.7:1）
  - `text-muted/40`: 装飾セパレータ（×・/ 記号）で WCAG 対象外
  - `text-foreground/60`: 約 5.2:1 で AA クリア（追加修正不要）
