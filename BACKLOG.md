...
### 51. GRIMOIRE 関連記事の拡張（件数 + スコア順）
- 2026-04-21 再評価由来（効率順 3位、+0.35、収益動線 #9 にも加点効果）
- 現状: 同カテゴリ関連4件（#13 対応済）
- 案:
  - 件数を 4→8 に拡張
  - スコア順（タグ一致度 + カテゴリ一致 + 著者一致 + 新しさ）でソート
  - `src/lib/stories.ts` の `getRelatedStories()` を強化
...
### 53. 利用規約独立ページ + 未成年対応
- 2026-04-21 再評価由来（効率順 5位、+0.30）
- 現状: 利用規約相当の記述は legal ページに混在
- 案:
  - `/terms` 新設（利用規約を独立ページ化）
  - legal から該当セクションを抽出 + 未成年対応注記を追加
  - Footer / サイトマップ / legal から相互リンク
  - 評価14 法令対応の -8（独立規約なし）+ -3（未成年対応）= -11 回収
